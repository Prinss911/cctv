    ...
    <table class="table table-hover">
        ...
        <?php foreach ($items as $item): ?>
            <tr data-id="<?= $item['id'] ?>">
                ...
                <td><div class="d-flex gap-1"><a href="<?= url('/admin/gallery/'.$item['id'].'/edit') ?>" class="btn-act"><i class="fas fa-pen"></i></a><form method="POST" action="<?= url('/admin/gallery/'.$item['id'].'/delete') ?>" class="delete-form"><?= \App\Helpers\Csrf::field() ?><button type="submit" class="btn-act danger"><i class="fas fa-trash-can"></i></button></form></div></td>
            </tr>
        <?php endforeach; ?>
        </tbody>
    </table>
