...
<div class="admin-card"><div class="card-body">
    <form method="POST" action="<?= url('/admin/gallery/'.$item['id']) ?>" enctype="multipart/form-data" class="admin-form">
        ...
        <div class="row">
            <div class="col-md-4 mb-3"><label class="form-label">Kategori</label><select class="form-select" name="category"><option value="instalasi" <?= $item['category']==='instalasi'?'selected':'' ?>>Instalasi</option><option value="produk" <?= $item['category']==='produk'?'selected':'' ?>>Produk</option><option value="lainnya" <?= $item['category']==='lainnya'?'selected':'' ?>>Lainnya</option></select></div>
